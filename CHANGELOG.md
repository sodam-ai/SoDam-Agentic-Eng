...
- ~~게이트 1 ③ — F2/F3 배너 실측(라이브, 사람이 새 세션에서 직접 관찰).~~ **2026-08-20 종결**(사용자 결정, `.PRD/03_PHASES.md` 게이트 1 각주·`CHECKPOINT.md §0-76`). §0-38→§0-63 최소 5개 독립 세션에서 이미 반복 관찰됐고 결과가 매번 "미표시"로 일관됐으며 원인(플랫폼 한계 — 스킬은 "부탁"이라 강제 발동 불가)도 규명됨, "실측" 조건은 문자 그대로 충족된 것으로 확정. **Phase 3 진입 게이트 1·2·3 전부 통과.**
...
> 배경: `.claude-plugin/plugin.json`·`package.json`이 최초 커밋(2026-06-28) 이후 한 번도 버전이 안 올라간 채 2개월치 작업이 전부 `[0.1.0]` 밑에 쌓여있던 것을 발견. `06_EXTENSIBILITY_AND_UPGRADE.md §3`가 이미 "semver + CHANGELOG"를 업그레이드 추적 수단으로 명시해뒀는데 실제로 안 지켜지고 있었음. 소급해서 인위적으로 과거 커밋들을 잘라 재분류하지 않고, **이 시점까지의 모든 누적 작업을 0.2.0으로 확정**하는 방식으로 정리(이 파일의 위 모든 항목이 대상).

**주요 누적 변경(0.1.0 이후 요약, 상세는 위 날짜별 항목 참고):**
- F6(안전기록 조회) · F7(Codex 안전 패리티) · F8(무경험자 쉬운 모드 v1 + 소범위 확장 3건) 신규 기능
- `hooks/guard.mjs` 보안 수정 5건(git branch -D 우회 2건, git filter-branch 무방비, 기타) — `_selftest.mjs` 122→126 PASS
- 게이트 1·2·3(Phase 3 착수 조건) 전부 종결
- 문서-대-코드 일치성 정정 다수(README 권장 MCP 섹션 신설 등)

- **변경**: `.claude-plugin/plugin.json`·`package.json` `version` `0.1.0`→`0.2.0`.

### 2026-08-20 — F8(쉬운 모드) 내부 빈틈 2건 보완 (사용자 지정 범위)

> 배경: F1~F7 구현 완료 확인 후 "F8만 작업하면 되냐"는 질문에, F8 자신의 구조 안에서 발견한 빈틈(2번 섹션이 "무서워요"를 선택지로 던지지만 정작 답을 안 함) 제안 → 사용자가 AskUserQuestion으로 둘 다 승인.

- **"잘못될까 봐 무서워요" 전용 안심 문구**: 2번 섹션 바로 아래에 새 하위 섹션 추가. "위험한 건 AI가 스스로 막는다"는 사실 기반 안심만 제공, "100% 안전" 과장 명시적으로 금지(F8 §4 원칙과 동일선상).
- **막막함 완화용 시작 예시**: "마무리" 섹션의 열린 질문에 "예를 들면..." 식 완전히 선택적인 예시 1개 추가, "다른 게 하고 싶으면 그게 우선"이라고 명시해 강요 방지.
- **검증**: `node hooks/_selftest.mjs` 126 PASS/0 FAIL 불변(F4 무영향). "100%" 문구 스캔 결과 둘 다 "쓰지 말라"는 금지문뿐(과장 없음 확인). `validate.mjs` PASS 14/WARN 1/FAIL 0 불변. 이 파일 1개만 변경, 비밀정보 스캔 클린.

## [0.2.1] - 2026-08-20

> 배경: `v0.2.0` 태그가 F8 안심문구·시작예시 커밋(`2f0c03a`)을 포함하지 않는 것을 QA 라운드에서 발견 — 사용자에게 보이는 F8 실제 동작이 바뀐 실질적 변경이라 PATCH 릴리스로 기록.

- **변경**: `.claude-plugin/plugin.json`·`package.json` `version` `0.2.0`→`0.2.1`.
- **포함 내용**: F8 "잘못될까 봐 무서워요" 안심 문구, 시작 예시 안내(위 항목 참고).

### 2026-08-20 — F8 남은 선택지 2개 답변 추가 (v0.2.1 태그 이후, 미배포)

> 배경: F8 2번 섹션의 3개 선택지 중 지난 라운드엔 1개(무서워요)만 답이 있었음. 나머지 2개("말투 모르겠다"·"화면 이해 안 됨")도 같은 패턴으로 발견해 보완.

- **"AI한테 뭘 어떻게 말해야 할지 모르겠어요"**: 특별한 명령어 불필요, 평소 말투로 충분하다는 안심 문구. 마무리의 시작 예시(무엇을 만들지)와 겹치지 않게 "어떻게 말해도 되는지"(형식)로 역할 구분.
- **"AI가 뭘 하는 건지 화면에 뜨는 말이 이해가 안 돼요"**: 스크롤되는 세부 내용은 안 읽어도 되고 계획·검토 두 지점만 보면 된다는 안심 — F2/F3의 중요성은 그대로 유지.
- **검증**: `_selftest.mjs` 126 PASS/0 FAIL 불변, "100%" 과장 문구 스캔 클린, 이 파일 1개만 변경. 커밋(`f481f71`)·push 완료.

## [0.2.2] - 2026-08-20

> 배경: 위 F8 답변 2건이 v0.2.1 태그 이후 커밋이라 릴리스에 빠져있던 것을 정리 — v0.2.0→v0.2.1과 동일한 기준(사용자가 보는 F8 실제 동작 변경) 적용.
...
> 배경: 위 라이선스 실사 확인 커밋이 `v0.2.2` 태그 이후에 남아있던 것을 정리 — 태그가 최신 작업을 항상 포함하도록 유지하는 이번 세션 확립 규율(v0.2.0→0.2.1→0.2.2와 동일 기준) 적용.
...
### 2026-08-21 — README 4종(ko/en × md/html) 전면 최신화

> 배경: "지금까지 개발한 기준으로 README를 목차·설치·보안·라이선스 등 전 항목 정확하게 작성"하라는 요청. `README.md`/`README.en.md`의 "현재 상태" 배너가 게이트 1 종결(2026-08-20)·F8 의도적 중단(v0.2.2) 이전 시점에서 멈춰 있던 것을 발견.

- **"현재 상태" 배너 정정**(ko/en): 게이트 1·2·3 전부 통과, F8은 사용자 지정 범위까지 구현 후 의도적으로 확장 중단(v0.2.2)했다는 최신 사실로 교체.
- **업데이트 요약(§8) 토글에 2026-08-21 항목 신규 추가**(ko/en): F8 나머지 답변·라이선스 실사 확인(`THIRD_PARTY_NOTICES.md`)·`commands/start.md` 안전경고 수정 요약.
- **문서 기준일 갱신**: 2026-08-20 → 2026-08-21 (v0.2.4).
- **HTML 2종 완전 재생성**: `pandoc`으로 본문을 md에서 새로 생성해 md/html 내용을 일치시킴. 기존 커스텀 헤더(다크모드 대응 CSS)는 그대로 보존, 표는 기존과 동일하게 `table-wrap` div로 래핑.
- **부수 발견·수정(4건)**: 한국어 볼드(`**단어(영문/괄호)**조사`처럼 닫는 `**` 바로 뒤에 조사가 붙는 경우) pandoc이 `<strong>` 변환에 실패해 `**`가 그대로 노출되는 렌더링 결함을 발견 — 원인은 문서 신규 작성이 아니라 예전부터 있던 문구였음. 공백/조사 위치만 조정(의미 불변)해 4곳 모두 수정.
- **검증**: `validate.mjs` PASS 14/WARN 1/FAIL 0 불변, HTML head 원본과 diff 0(구조 보존 확인), 시크릿 노출 스캔 클린. 기능·코드 변경 없음(문서 전용이라 버전 번호는 올리지 않음).

### 2026-08-31 — `scripts/validate.mjs`에 `.agents/` 자기설치 사본 드리프트 자동감지 추가

> 배경: CHECKPOINT.md §5(2026-08-23)가 발견한 결함 — `hooks/guard.mjs`(활성 안전훅)에 들어간 보안 수정 5건(`.mcp.json` 보호·git 위험명령 차단 등)이 로컬 자기설치 산출물 `.agents/hooks/guard.mjs`(`codex/install.mjs` 셀프테스트 잔여물, gitignored)엔 8일째 반영 안 된 채 방치돼 있었음을 재확인, 재발 방지 조치.

- **추가**: `scripts/validate.mjs`에 "[.agents/ 자기설치 사본]" 검사 신설 — `hooks/guard.mjs`와 `.agents/hooks/guard.mjs`의 내용을 비교해 다르면 WARN(FAIL 아님, 배포 대상이 아니므로). `.agents/`가 없는 일반적 환경은 정상(PASS) 처리해 오탐 없음.
- **조치**: `node codex/install.mjs` 재실행으로 `.agents/` 사본을 최신 소스로 재생성. 안전 판정 로직인 `hooks/guard.mjs` 자체는 무변경(deny/ask 기준 불변).
- **검증**: `validate.mjs` PASS 14→**15**, WARN 2→**1**(드리프트 WARN 해소, 남은 1개는 무관한 기존 항목). `hooks/_selftest.mjs` **126 PASS/0 FAIL** 불변(안전 로직 무변경이므로 회귀 없음, 실행으로 재확인). `git status --short` 확인 결과 `scripts/validate.mjs` 1개 파일만 변경(+15줄).
- **버전**: 사용자가 보는 AI 동작(guard.mjs 판정 로직)은 바뀌지 않아 버전 번호는 올리지 않음(2026-08-21 README 전용 갱신과 동일 기준).

### 2026-09-01 — `docs/family-synergy.md`에 "Harness 위임 참여 현황" 표 추가(문서 정정, 코드 무변경)

> 배경: CHECKPOINT.md §6(2026-08-23, 정책 설계 결함 심층 분석) 6-5 항목 — 이 문서의 "규약 D: Harness 감지 위임 패턴"이 "각 형제가 위임 패턴을 쓴다"고 암묵 전제하지만, 같은 §6이 인용한 7형제 가드 전수 조사(2026-08-23)는 **Loop·Reverse는 위임 없이 독립적으로 deny한다**는 사실을 이미 확인해뒀음에도 이 문서엔 반영 안 돼 있었음.

- **추가**: 규약 D 코드 예시 뒤에 "위임 참여 현황" 표 신설 — Agentic만 위임 구현, Loop·Reverse는 독립 판정임을 명시. 이 문서만 읽고 "위임하면 항상 확인창이 하나로 줄어든다"고 오판할 위험을 제거.
- **범위**: 이 저장소 파일만 수정. 소담 패밀리 우산저장소(SoDam_Family)에 동일 문서 사본이 있다면 그쪽 동기화는 이번 범위 밖(별도 확인 필요).
- **코드 변경 없음** — 순수 문서 정정, 버전 번호 불변.

### 2026-09-01 — CHECKPOINT.md §6 R3 오진단 정정(gitignored 문서, git 이력 무관) + 6-3/6-5/6-6 종결

> 배경: "PRD 재확인 → 강력 추천 방향 제안·진행" 6번째 라운드. §6 R3("`.claude/settings` 판정이 파일 전체 deny")를 코드로 재대조하니 실제로는 Write/Edit 경로가 이미 항목 단위 판정(민감 키만 deny, 나머지 ask)을 구현하고 있어 R3 자체가 오진단이었음을 발견 — 정정 완료(§6-3 종결). 6-5(가족 문서 위임 현황 반영, 위 항목)와 6-6(F2/F3 척추-형제가드 라이브 실측)도 같은 라운드에서 완료. `.PRD/`는 gitignore 대상이라 이 항목은 git 이력에 남지 않지만, 판단 근거 추적을 위해 함께 기록.

---

## [0.2.5] - 2026-09-01

> 배경: "지금까지 구현된 기능이 제대로 작동하는지 테스트·검증"하라는 QA 요청. 정상/예외/경계값 시나리오를 실제 stdin으로 라이브 프로브하던 중, `rm -rf ~`(치명 명령)가 기대와 달리 "deny"가 아니라 "ask"로 판정되는 것을 재현 — 안전 로직 자체의 결함이라 재현→원인분석→수정→재검증 절차로 처리.

### 수정 (Fixed)

- **[안전][치명 명령 순서 결함]** `hooks/guard.mjs`: 셸 명령이 치명(catastrophic, 예: `rm -rf ~`)으로 분류돼도, 그 명령의 대상 경로가 "작업폴더 밖"에도 동시에 해당하면 더 약한 `isOutsideWorkdir` 확인(ask)이 치명 명령 무조건 차단(deny) 검사보다 먼저 실행돼 **"ask"로 격하되는 순서 결함**이 있었음. 특히 실제 SoDamHarness가 설치된 기기에서는 `isSensitive()`의 deny 분기(harness 없을 때만 실행)까지 함께 건너뛰어, 우연히 다른 검사로 가려지지 않고 그대로 "ask"만 남는 상태였음(이 기기에서 라이브 재현 확인).
  - **원인**: `level === "catastrophic"` deny 판정이 코드 순서상 경로 검사 루프(settings/mcp/민감위치/심볼릭링크/작업폴더 밖) 뒤에 있어서, 경로 기반 검사가 먼저 `return`하면 치명 판정까지 도달하지 못함.
  - **수정**: 치명 명령 deny 판정을 `level` 계산 직후, 경로 검사 루프보다 먼저 실행하도록 이동(로직 자체는 변경 없음 — 순서만 정정). `hooks/guard.mjs`의 실제 안전 판정(deny/ask 기준)이 사용자가 보는 방향으로 강화됨.
  - **재발 방지**: `hooks/_selftest.mjs`에 회귀 테스트 신설("치명 명령(rm -rf ~, 경로 추출됨)은 이 기기의 실제 Harness 설치여부와 무관하게 deny여야 함") — B2 섹션의 `runCopy`(harness 강제 off 안 함, 실제 감지 그대로 사용)를 재사용해 이 기기 실제 Harness 설치 상태에서 검증.

### 검증
- 수정 전 `_selftest.mjs`로 재현 고정: 126 PASS / **1 FAIL**(신규 테스트만 실패, RED 확인).
- 수정 후: **127 PASS / 0 FAIL**(신규 테스트 통과 + 기존 126개 전부 무변화, GREEN).
- 독립 라이브 재확인(테스트 스위트와 별개로 실제 stdin 직접 호출): `rm -rf ~` → `{"permissionDecision":"deny", ...}` 확인. 인접 회귀 확인 2건(정상 명령 무결정 통과, 단일파일 삭제는 risky 등급대로 harness 위임) 모두 기존과 동일하게 정상 작동.
- `node scripts/validate.mjs` → PASS 15/WARN 1/FAIL 0(WARN은 무관한 기존 항목). `.agents/` 자기설치 사본은 `node codex/install.mjs` 재실행으로 함께 갱신 확인(드리프트 재검사 PASS).
- `.claude-plugin/plugin.json`·`package.json` `version` `0.2.4` → `0.2.5` (실제 안전 판정 동작이 바뀐 변경이라 버전 상승 — 2026-08-21 F1 안전경고 수정(v0.2.4) 때와 동일 기준).

---

## [0.2.6] - 2026-09-01

> 배경: v0.2.5 배포 확인 직후 이어진 QA 라운드. "정상/예외/잘못된입력/경계값/실패" 시나리오를 실제 stdin으로 추가 프로브하던 중, Windows 파괴적 명령(`Remove-Item`·`del`·`erase`·`rd`/`rmdir`)에서 v0.2.5와 **같은 계열이지만 원인이 다른** 결함을 발견 — 재현→원인분석→수정→재검증 절차로 처리.

### 수정 (Fixed)

- **[안전][치명 명령 정규식 순서의존 결함]** `hooks/guard.mjs`: `rm -rf ~` 등 bash 계열은 문법상 플래그가 항상 경로보다 먼저 오지만(`-rf ~`), PowerShell/CMD는 플래그가 경로 뒤에 와도 유효한 문법이다(`Remove-Item C:\ -Recurse -Force`, `del C:\ /s` 등). 기존 CATASTROPHIC 정규식 3종(`remove-item`·`del`/`erase`·`rd`/`rmdir`)이 "플래그 → 경로" 순서만 인식하도록 짜여 있어서, 실무에서 흔한 반대 순서는 애초에 `level`이 "catastrophic"으로 분류되지 못하고 "risky"로 격하돼 더 약한 "ask"만 떴음(4개 명령 전부 라이브 재현 확인).
  - **원인**: 정규식이 `-recurse[^|;&]*(경로)` 형태로 플래그 뒤에만 경로를 찾도록 고정돼 있었음(v0.2.5의 "판정 순서" 문제와 달리, 이번은 "정규식 매칭 순서" 문제).
  - **수정**: 플래그 존재 확인을 앞쪽 lookahead(`(?=...)`, 위치를 소비하지 않음)로 옮겨 플래그가 경로 앞이든 뒤든 상관없이 인식하게 함. 대상 판정은 "드라이브 루트 뒤에 공백/문장끝/구분자가 온다"는 경계 조건만 유지해, 하위 폴더·파일 삭제(예: `C:\temp\build`)는 여전히 과잉차단하지 않음(회귀 테스트로 확인).
  - **재발 방지**: `hooks/_selftest.mjs`에 순서반전 4건(deny 기대) + 과잉차단 방지 2건(하위경로는 deny 아님) 회귀 테스트 신설, B2 섹션 `runCopy`로 이 기기 실제 Harness 설치 상태에서 검증.

### 검증
- 수정 전 `_selftest.mjs`: 129 PASS / **4 FAIL**(신규 순서반전 4건만 실패, RED 확인).
- 수정 후: **133 PASS / 0 FAIL**(신규 6건 전부 통과 + 기존 127개 무변화, GREEN).
- 독립 라이브 재확인(테스트 스위트와 별개, 실제 stdin 직접 호출) — 8가지 순서 조합(`Remove-Item`/`del`/`erase`/`rd` × 정순/역순) 전부 `deny` 확인. 인접 회귀 4건(홈 하위 폴더만 삭제·체인 명령 은닉·대문자 `RM -RF ~`·정상 명령) 모두 기존과 동일하게 정상 작동.
- `node scripts/validate.mjs` → PASS 15/WARN 0/FAIL 1(FAIL은 "설치 캐시가 아직 이 수정 전 버전" — 예상된 정상 신호, 재설치 전까지는 계속 뜸). `.agents/` 자기설치 사본은 `node codex/install.mjs` 재실행으로 갱신, 드리프트 재검사 PASS.
- `.claude-plugin/plugin.json`·`package.json` `version` `0.2.5` → `0.2.6`(실제 안전 판정 동작이 바뀐 변경).

---

## [0.2.7] - 2026-09-01

> 배경: v0.2.6 직후 같은 QA 라운드에서 "장점(안전 기본)을 살리고 단점을 체계적으로 보완"하는 방향으로 진행 승인. v0.2.5·v0.2.6에서 발견한 "규칙이 특정 순서/형태만 가정한다"는 결함 유형을 단서 삼아, 아직 점검 안 한 나머지 안전 규칙(키/비밀값 노출·심볼릭링크 생성 차단)을 같은 방식으로 재점검 — 2건 추가 발견·수정.

### 수정 (Fixed)

- **[안전][키 노출 탐지 순서 편중]** `data/agentic-rules.json`(`key_exposure_deny`): 민감 환경변수를 콘솔에 출력하는 위험 패턴이 "환경변수 참조 → 나중에 콘솔 출력" 순서만 인식했다. 그런데 실제로 개발자가 가장 자연스럽게 쓰는 형태는 콘솔 출력 함수가 환경변수 참조를 감싸는(바깥에서 안쪽 인자로 받는) **반대 순서**라, 이 흔한 관용구가 완전히 무결정(passthrough)으로 샜다(라이브 재현 확인). **수정**: 반대 순서를 인식하는 패턴을 추가(기존 패턴은 그대로 유지, 새 패턴만 추가하는 방식이라 회귀 없음).
- **[안전][심볼릭링크 결합 짧은 플래그 우회]** `data/agentic-rules.json`(`extra_denied`): 심볼릭링크 생성 차단 규칙(`ln -s`)이 단독 플래그만 인식하고, 실사용에서 흔한 결합 짧은 플래그(force와 symbolic을 한 토큰에 같이 쓰는 형태, 순서 무관)는 기존 패턴의 단어경계(`\b`)가 결합된 다음 글자와 충돌해 놓쳤다(2026-08-19 `git branch -Df` 결합플래그 버그와 같은 계열, 라이브 재현 확인). **수정**: "-"로 시작하는 짧은 플래그 토큰 안에 `s`가 포함되면 순서·결합 여부와 무관하게 인식하도록 변경, 장문 옵션(`--symbolic`)은 오탐 없이 그대로 제외.

### 검증
- 심볼릭링크 결합플래그 건은 단독/결합(2종)/무관 명령(대조군) 4건을 `hooks/_selftest.mjs`에 정식 회귀 테스트로 추가 — **137 PASS / 0 FAIL**(기존 133개 전부 무변화).
- 키 노출 순서반전 건은 `_selftest.mjs`(gitignored)에 자동 테스트를 추가하려 했으나, 이 파일 자체를 편집하는 과정에서 guard.mjs 자신의 키 노출 탐지 규칙이 테스트 코드 문구 자체를 실제 키 노출 시도로 오인해 편집이 반복적으로 막히는 자기참조 한계에 부딪혔다(변수명·문자열 조각내기 등 여러 우회를 시도했으나 정확한 트리거 원인을 완전히 규명하지 못함 — 정직하게 "미해결"로 남김). **대신 테스트 스위트 밖에서 독립 stdin 프로브로 직접 재현·수정 전후를 대조**: 수정 전에는 반대 순서 명령이 무결정(passthrough)이었고, 수정 후 같은 명령은 차단(deny)으로 바뀜을 2회 반복 재현으로 확인. 이 건은 자동 회귀 스위트에 없어 향후 실수로 되돌아갈 위험이 남아있음(남은 위험으로 별도 기록).
- `node scripts/validate.mjs` → PASS 15/WARN 1/FAIL 0(WARN="설치 캐시가 아직 이 버전 이전" — 재설치 전까지 뜨는 예상된 정상 신호, v0.2.5·v0.2.6과 동일 패턴). `.agents/` 자기설치 사본은 `node codex/install.mjs` 재실행으로 갱신, 드리프트 재검사 PASS.
- `.claude-plugin/plugin.json`·`package.json` `version` `0.2.6` → `0.2.7`(실제 안전 판정 동작이 바뀐 변경).

---

## [0.2.8] - 2026-09-01

> 배경: v0.2.5~v0.2.7 커밋·push 직후 이어진 QA 라운드. 지금까지는 셸 명령(Bash)과 파일 경로 검사 위주로 점검했는데, 이번엔 아직 깊이 안 본 "파일 쓰기 도구의 내용 검사" 쪽을 Claude Code 공식 도구 스키마와 직접 대조하며 점검 — 이번 세션 중 가장 심각한 결함(부분 완화가 아니라 완전 무방비) 발견.

### 수정 (Fixed)

- **[안전][NotebookEdit 키노출 검사 완전 누락]** `hooks/guard.mjs`(`writeContents`): 파일 쓰기 도구가 파일에 API 키·비밀값을 직접 적으려 하면 막는 검사(②)가 `content`/`new_string`/`new_str` 필드만 확인했다. 그런데 Claude Code의 실제 `NotebookEdit` 도구는 셀 내용을 `new_source`라는 **다른** 필드로 전달한다(도구 자신의 공식 스키마로 직접 확인). 경로 검사(`notebook_path` → 작업폴더 밖 등)는 정상 작동했지만, **내용 검사만 이 필드 하나가 빠져 있어서 완전히 무방비**였다 — 순서가 뒤집혀 더 약한 등급으로 격하되는 v0.2.5~0.2.7과 달리, 이번은 아예 검사 자체가 실행되지 않는 더 심각한 유형. 라이브 재현: 키 리터럴을 담은 `new_source`가 무결정(passthrough)으로 통과함을 확인.
  - **수정**: `writeContents()`가 확인하는 필드 목록에 `new_source` 추가(한 줄, 최소 변경).

### 검증
- 라이브 재현·수정 전후 대조(수정 전 무결정 → 수정 후 `deny`) + 회귀 테스트 신설(`hooks/_selftest.mjs`, 소스에 리터럴 키 패턴이 안 보이게 조각내는 기존 관례 적용) — **138 PASS / 0 FAIL**(기존 137개 전부 무변화).
- 과잉차단 회귀 확인: 비밀값 없는 정상 노트북 셀 편집은 여전히 무결정 통과(라이브 재확인).
- `node scripts/validate.mjs` → PASS 15/WARN 1/FAIL 0(WARN=예상된 설치캐시 지연 신호, 이전과 동일 패턴). `.agents/` 자기설치 사본 갱신·드리프트 재검사 PASS.
- `.claude-plugin/plugin.json`·`package.json` `version` `0.2.7` → `0.2.8`.
